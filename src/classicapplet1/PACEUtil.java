...
public class PACEUtil implements ISO7816 {
   ...
   */
  public static void xor(byte[] in1, short in1_o, byte[] in2, short in2_o, byte[] out, short out_o, short len) {
  ...
  /**
   * Swaps two non-overlapping segments of the same length in the same byte
   * array in place.
   *
  ...
  /**
   * *
   * Pads an input buffer with max 16 and min 1 byte padding (0x80 followed by
   * optional zeros) relative to the offset and length given. Always pad with
   * at least a 0x80 byte.
   *
   ...
   *
   */
  public static short padPACE(byte[] buffer, short offset, short len) {
    short padbytes = (short) (lengthPACEWithPadding(len) - len);
  ...
  public static short lengthPACEWithPadding(short inputLength) {
    return (short) ((((short) (inputLength + 16)) / 16) * 16);
  }

  /**
   * *
   * Computes the actual length of a data block as byte value, without the
   * padding.
   *
  ...
  public static byte calcLcFromPaddedData(byte[] apdu, short offset, short length) {
    for (short i = (short) (length - 1); i >= 0; i--) {
      if (apdu[(short) (offset + i)] != 0) {
        if ((apdu[(short) (offset + i)] & 0xff) != 0x80) // not padded
        {
